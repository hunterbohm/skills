...
description: "Write a personalized Workflow Roadmap for a business owner or builder: a roughly ten-minute interview, then a local file mapping their recurring workflows in their own numbers, with one clear pick for what to automate first. Use when the user asks for a workflow roadmap, asks what they should automate first or hand to an agent, or types /workflow-roadmap."
---
...
Keep every sentence plain enough for an 8th grader — short sentences, plain words.
...
Ask conversationally in small batches of two or three related questions per message — never all ten as a form. Skip anything the conversation already answered. Cover this ground:
...
7. Roughly what an hour of their time is worth. A range is fine; "prefer not to say" is a valid answer.
...
Write the document to `Workflow-Roadmap-{company}.md` in the current directory (or `Workflow-Roadmap.md` if there is no company name). Copy the skeleton in `references/roadmap-template.md` exactly — it owns the title, length, headings, and the closing line, which is the only place Hunter appears. Fill the five sections by these rules:

**1. Your workflow map.** From their answers, list every distinct recurring workflow you can identify (aim for 4–8). For each: what triggers it, what it produces, and your estimate of hours per month it costs them. Show the math from THEIR numbers — how often × how long — and if they gave an hourly value, show dollars per year too.
...
**3. What "runs itself" would look like.** For the #1 workflow: a concrete day-in-the-life paragraph — the trigger fires, the agent does the work, a draft lands for approval, the log records it. Ground it in the actual tools they named.

**4. If you build it yourself.** 3–5 honest bullets: the pieces (an agent runtime, connections to their tools, an approval step), the real time cost (about a working week to stand up, 2–3 hours a week to keep current), and one pitfall specific to their situation. This section is REAL advice — it must be genuinely usable without buying anything.

**5. The math.** One honest paragraph: workflow #1 costs them about N hours a month (about $X a year at their rate), and getting it off their plate is worth $Y a year, every year after — their numbers only.

Completion criterion: the file matches the template skeleton — all five sections and the closing line — and every number traces to the notes file.
...
- Round every estimate down. An honest small number beats an impressive one.
